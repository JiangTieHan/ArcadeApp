...
void Screen::Draw(const AARectangle& rect, const Color& color, bool fill, const Color& fillColor)
{
	...
	const std::vector<Vec2D>& points = rect.GetPoints();
...
void Screen::FillPoly(const std::vector<Vec2D>& points, const Color& color)
{
	if (!points.empty())
	{
		...
		float bot = points[0].GetY();
		float right = points[0].GetX();
		float left = points[0].GetX();

		for (size_t i = 1; i < points.size(); i++)
		{
			const Vec2D& point = points[i];
			top = fminf(top, point.GetY());
			bot = fmaxf(bot, point.GetY());
			right = fmaxf(right, point.GetX());
			left = fminf(left, point.GetX());
		}

		for (int pixelY = top; pixelY < bot; pixelY++)
		{
