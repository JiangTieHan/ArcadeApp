...
void InputController::Update(uint32_t dt)
{
	...
	while (SDL_PollEvent(&sdlEvent))
	{
		...
		case SDL_MOUSEMOTION:
			...
			{
				if (MouseMovedAction mousedMoved = mnoptrCurrentController->GetMouseMovedAction())
				{
					MousePosition position;
					position.xPos = sdlEvent.motion.x;
					position.yPos = sdlEvent.motion.y;
					mousedMoved(position);
				}
			}
			break;
		...
		case SDL_MOUSEBUTTONDOWN:
			if (mnoptrCurrentController)
			{
				MouseInputAction action = mnoptrCurrentController->
					GetMouseButtonActionForMouseButton(static_cast<MouseButton>(sdlEvent.button.button));
				MousePosition position;
